...
import { RUNTIME_EVENTS } from './runtime-event-names.js';
...
export async function setupListeners() {
  const u1 = await listen(RUNTIME_EVENTS.TASK_PROGRESS, (event) => {
  ...
  const u2 = await listen(RUNTIME_EVENTS.QUEUE_STATE_CHANGED, async () => {
    flushPendingProgress();
    await loadQueueState();
  });

  const u3b = await listen(RUNTIME_EVENTS.TASK_TERMINAL_COMMITTED_LINE, (event) => {
    const payload = event.payload;
    queueTerminalCommittedLine(payload.id, payload.line ?? '');
  });

  const u3c = await listen(RUNTIME_EVENTS.TASK_TERMINAL_ACTIVE_LINE, (event) => {
    const payload = event.payload;
    queueTerminalActiveLine(payload.id, payload.activeLine ?? '');
  });

  const u4 = await listen(RUNTIME_EVENTS.HISTORY_TASK_ADDED, (event) => {
    const payload = event.payload;
    prependHistoricalTask(payload.status, payload.task);
  });

  const u5 = await listen(RUNTIME_EVENTS.SHUTDOWN_COUNTDOWN_STARTED, (event) => {
    const seconds = Number(event.payload?.seconds ?? 60);
    startShutdownCountdown(seconds);
  });

  const u6 = await listen(RUNTIME_EVENTS.SHUTDOWN_COUNTDOWN_CANCELLED, () => {
    clearShutdownNotice();
  });

  const u7 = await listen(RUNTIME_EVENTS.SHUTDOWN_ERROR, (event) => {
    shutdownNotice.set({
      active: false,
      secondsRemaining: 0,
  ...
  const u8 = await listen(RUNTIME_EVENTS.TASK_ERROR, (event) => {
