<script>
  ...
  let liveCommittedLines = $state([]);
  let liveActiveLine = $state('');
  let hasLiveActiveLine = $state(false);
  const mergeCommittedLines = createTerminalCommittedLineMerger();
  ...
  let mergeBaseCommittedLines = $derived(capRenderedTerminalLines(committedLines));
  let mergedCommittedLines = $derived(
    mergeCommittedLines(mergeBaseCommittedLines, liveCommittedLines ?? [])
  );
  ...
  let visibleLineCount = $derived(renderedCommittedLines.length + (displayActiveLine ? 1 : 0));
  let displayLineCount = $derived(
    Math.max(cliOutputTotal ?? 0, committedLines.length, mergedCommittedLines.length) +
      (displayActiveLine ? 1 : 0)
  );
  let hiddenRenderedLineCount = $derived(
    Math.max(0, displayLineCount - visibleLineCount)
  );
  let scrollContentSignature = $derived(
    `${visibleLineCount}:${renderedCommittedLines[renderedCommittedLines.length - 1] ?? ''}:${displayActiveLine}`
  );
  ...
  async function loadEarlierCliOutput() {
    ...
    try {
      ...
      if (!shouldApplyCliOutputPageResponse({
        responseTaskId: taskId,
        currentTaskId: task?.id,
        requestId,
        activeRequestId: terminalLoadState.requestId,
      })) return;
  ...
  $effect(() => {
    ...
    const unsubscribeCommitted = subscribeTerminalCommittedLines(taskId, (lines) => {
      liveCommittedLines = lines;
    });
    const unsubscribeActive = subscribeTerminalActiveLine(taskId, (activeLineState) => {
      hasLiveActiveLine = activeLineState.hasValue;
      liveActiveLine = activeLineState.line;
    });
  ...
  $effect(() => {
    const _trigger = scrollContentSignature;
