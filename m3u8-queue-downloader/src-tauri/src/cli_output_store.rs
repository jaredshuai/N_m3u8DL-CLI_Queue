...
impl CliOutputStore {
    ...
    fn load_index_for_append(&self, task_id: &str) -> Result<CliOutputIndex, String> {
        let task_dir = self.task_dir(task_id);
    ...
    fn load_index(&self, task_id: &str) -> Result<CliOutputIndex, String> {
    ...
    fn save_index_atomic(&self, task_id: &str, index: &CliOutputIndex) -> Result<(), String> {
        let path = self.index_path(task_id);
...
fn load_chunk_sequence_from_disk(task_dir: &Path) -> Result<CliOutputIndex, String> {
    if !task_dir.exists() {
...
fn append_chunk_line(path: &Path, line: &str) -> Result<(), String> {
...
fn read_chunk_lines(path: &Path) -> Result<Vec<String>, String> {
...
fn count_chunk_lines(path: &Path) -> Result<usize, String> {
    let file = fs::File::open(path).map_err(|e| e.to_string())?;
    BufReader::new(file)
        .lines()
